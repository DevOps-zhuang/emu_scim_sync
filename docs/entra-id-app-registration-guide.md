...
<img src="media/entra-id-app-registration-guide/overall-implementation-sequence.svg" alt="Overall implementation sequence" width="1200" />
...
### 4.2 In the Entra portal, switch to SAML and fill the fixed URL patterns
...
### 4.3 In the Entra portal, export the SAML output values

Everything in this subsection happens inside the 21V Entra Enterprise Application page. You are still not on the GitHub side yet.

Treat this subsection as Entra-only work: collect values here, but do not switch to GitHub yet.
...
### 4.4 In GitHub enterprise, enter the exported SAML values

This subsection switches from Entra to the GitHub enterprise admin page. Take the values exported in the previous subsection and fill them into GitHub.

Treat this subsection as GitHub-only work: do not return to Entra here unless you discover a value was copied incorrectly.

1. Open the SAML configuration page in your GitHub enterprise
2. Enter the Entra outputs into GitHub
...
To avoid operating in the wrong system, think of 4.2, 4.3, and 4.4 as three different responsibilities:

- In 4.2, you fill GitHub EMU fixed SP URL patterns in Entra
- In 4.3, you export the IdP outputs generated by Entra
- In 4.4, you apply those exported Entra values in GitHub enterprise
...
### 4.5 Claims and NameID alignment
...
### 4.6 In the Entra portal, assign a test user to the Enterprise Application
...
### 4.7 In GitHub enterprise, finish the post-authentication setup
...
## 12. Visual maintenance notes

The public guide now shows only final diagrams and real screenshots that are ready for external readers.

If more real portal screenshots are added later, prioritize these operator touchpoints:

1. The Entra Gallery search result for GitHub Enterprise Managed User
2. The GitHub enterprise SAML settings page
3. The Enterprise App test user assignment and Enterprise Owner role page
4. The App Registration Overview page with Tenant ID and Client ID
5. The Client Secret page showing the Value field
6. The API permissions page with Grant admin consent
7. The Entra Groups page with the exact displayName values used in ENTRA_SYNC_GROUP_NAMES
